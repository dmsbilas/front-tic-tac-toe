...
describe('Game Duck', () => {
  ...
  describe('Reducers', () => {
    ...
    it('should send/retrive data to/from backend', () => {
      const state = {
        board: [
          [0, 0, 0],
          [0, 0, 0],
  ...
  describe('Operations', () => {
    ...
    const { checkWinner, playTurn, apiOperations } = operations;
    ...
    it('should save Data to API', () => {
      ...
      const saveApiCall_1 = actions.saveToApi(givenState);
      const getApiCall_1 = actions.getFromApi();

      apiOperations(givenState)(dispatch);

      expect(dispatch).toHaveBeenCalledTimes(2);
      expect(dispatch.mock.calls[0][0]).toEqual(saveApiCall_1);
      expect(dispatch.mock.calls[1][0]).toEqual(getApiCall_1);
    });
