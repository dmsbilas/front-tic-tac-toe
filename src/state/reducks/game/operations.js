...
import { newGame, gameover, switchPlayer, winner, movePlayer, getFromApi,saveToApi } from './actions';
...
const apiOperations= (gameState) => (dispatch) =>{
  dispatch(saveToApi(gameState));
  dispatch(getFromApi());
};

const logData = (board, player) =>{
  dispatch(updateLogData(board, player));
};
